...
grouping_= 2 #for specifying time series grouping in Technology Aggregation
...
class PotentialsToNestorDB():
    ...
    def prepare_nestor_data(self, lifetime=20, weather_year=2014):
        ...
        self.grouped_historical_data_regions= pd.DataFrame()
        self.grouped_potentials = pd.DataFrame(columns=["name", "profile", "lb", "ub"])
        self.grouped_timeseries = pd.DataFrame()
        ...
        if self.technology=="openfield_pv":
            for region in regions:
                ...
                if self.region_availability[region]["potential"]:
                    self.spatial_Tech_aggregation( region, group_name, _region_path)
                    self.spatial_aggregation_prepare_historical_data(group_name,lifetime)
                    self.spatial_aggregation_prepare_potentials( region,group_name)
            ...
            self.grouped_timeseries.fillna(0).to_csv(
                os.path.join(self.nestor_ee_scenario_path, f"grouped_timeseries_{grouping_}.csv"))
            self.grouped_historical_data_regions.to_csv(
                os.path.join(self.nestor_ee_scenario_path, f"grouped_historical_{grouping_}.csv"))
            self.grouped_potentials.to_csv(
                os.path.join(self.nestor_ee_scenario_path, f"grouped_potentials_{grouping_}.csv"))
            ...
            if self.time_series.max().max() > 0:
                print(
                    f"WARNING: Values higher 1 for time-series. Max value {self.time_series.max().max()} Will be corrected to 1.")
                self.time_series = self.time_series.clip(upper=1)
            self.grouped_timeseries.to_csv(
                os.path.join(self.nestor_ee_scenario_path, f"grouped_timeseries_{grouping_}.csv"))
            self.grouped_historical_data_regions.to_csv(
                os.path.join(self.nestor_ee_scenario_path, f"grouped_historical_{grouping_}.csv"))
            self.grouped_potentials.to_csv(
    ...
    def prepare_FLH(self, _region_path, region, group_name):
        ...
        if not "FLH" in  potential_FLH.columns:
            potential_FLH["FLH"] = 0
        elif potential_FLH["FLH"].empty:
            potential_FLH["FLH"] = 0
        elif existing_FLH["FLH"].empty or "FLH" not in  existing_FLH.columns:
            existing_FLH["FLH"] = 0
        else:
        ...
        self.fullloadhours_stan[group_name] = energy/potential #this wasn't used in any part of the code
        # print(self.fullloadhours)
    ...
    def prepare_time_series(self, path, region, group_name, weather_year):
        ...
        timeseries = pd.read_csv(os.path.join(
            path, f"ts_{naming_conversion_mapping[self.technology]}_{region}_{weather_year}.csv"), index_col=0).sum(axis=1)
        
        timeseries_existing = pd.read_csv(os.path.join(
            path, f"ts_existing_{naming_conversion_mapping[self.technology]}_{region}_{weather_year}.csv"), index_col=0).sum(axis=1)
        ...
        if len(timeseries_existing.columns) > 1:
            raise NotImplementedError(
                "Currently only one time-series per region can be passed")
    ...
    def prepare_historical_data(self, path, region, group_name, lifetime):
        ...
        if self.technology=="rooftop_pv":
            ...
            if 'commissioning_year' in existing.columns:
                ...
                self.historical_data_regions[group_name] = 0
    ...
    def prepare_potentials(self, path, group_name, region):
            ...
            try:
                ...
                capacity = round(
                    (report["Capacity"]/1000000 +
                        self.historical_data_regions.loc[:, group_name].max()), 4)
            except:
        ...
        print(' capacity + max historical', capacity)
        print(f'maximum historical value in {group_name}:', self.historical_data_regions.loc[:, group_name].max())
        print(f' historical value in  2020 {group_name}:', self.historical_data_regions.loc[2020, group_name])
            # potential
        if self.region_availability[region]["potential"]:
    ...
    def _rooftop_potentials(self, rs):
        ...
        for a in sorted(path):
            ...
            if code.startswith(rs):
                ...
                trp.RooftopPV.sim()
                trp.RooftopPV.predicted_items.fillna(0)
                ...
                ts_existing_items = trp.RooftopPV.ts_existing_items.reindex(columns=column).fillna(0)
                if type(trp.RooftopPV.ts_predicted_items) != None:
                    ts_predicted_items = trp.RooftopPV.ts_predicted_items.reindex(columns=column).fillna(0)
                
                
                existing.append(trp.RooftopPV.existing_items.fillna(0))
                ...
                ts_existing.append(ts_existing_items)
                ts_potential.append(ts_predicted_items)
        ...
        print('list in mun: \n', municipality_list, ' \n', ts_potential)
        # # first method of processing to obtain coordinations
        ...
        #Another idea to processing the values of each municipality  
        if len(existing) >= 1:
            df_existing = pd.concat(existing)
        if len(potential) >= 1:
            df_potential = pd.concat(potential)

        df_ts_existing = pd.concat(ts_existing, axis=1).sum(axis=1)
        df_ts_potential = pd.concat(ts_potential, axis=1).fillna(0).sum(axis=1)

        df_ts_existing.columns = ['PV_items_000']
        df_ts_potential.columns = ['PV_items_000']
        
        if rs=='0o2' or rs==11 or len(potential) == 1:
        ...
        print(f'total potential timeseries data in region {rs}' , df_ts_potential )  
    ...
    def spatial_Tech_aggregation(self, region, group_name, _region_path):
        
        from trep import Technology
        
        total_existing_df = []  
        # print(_region_path)      
        for file in glob.glob(os.path.join(os.path.dirname(_region_path),'*/existing_*_*.csv')):
            df = pd.read_csv(file)
            total_existing_df.append(df)
        #print(total_existing_df)
        national_placements = pd.concat(total_existing_df)
        national_capacity = national_placements["capacity"].sum()
        #print(national_placements)

        potential_placements = pd.read_csv(os.path.join(_region_path,f'{naming_conversion_mapping[self.technology]}_{region}.csv'))
        ...
        if self.technology=='offshore' or self.technology=='onshore':
            #single region Wind potentials Technology aggregation 
            try:
                self.tech_agg_potential = Technology.sim_wind(potential_placements,
                ...
                self.tech_agg_potential[1].to_csv(os.path.join(self.nestor_ee_db_path, self.case_name, group_name, f'Aggregated_{group_name}_{grouping_}.csv'))
                self.tech_agg_potential[0].to_csv(os.path.join(self.nestor_ee_db_path, self.case_name, group_name, f'Aggregated_ts_{group_name}_{grouping_}.csv'))

                
                df = self.tech_agg_potential[1]
                ts_df= self.tech_agg_potential[0]

                
                for x in range(grouping_):
                    if F'Wind_items_00{x}' in ts_df.columns : 
                        self.grouped_timeseries[group_name +'_'+ F'Wind_items_00{x}'] = ts_df[F'Wind_items_00{x}']  / df.loc[df['group_2014'] == F'Wind_items_00{x}']["capacity"].sum()
            ...
            try:
                #single region Wind (existing) Technology aggregation and Timer series output
                self.tech_agg_existing= Technology.sim_wind(placements,
                                    grouping_method="spagat",
                                    n_groups=grouping_,
                                    turbine=None,
                                    year=2014,
                ...
                self.tech_agg_existing[1].to_csv(os.path.join(self.nestor_ee_db_path, self.case_name,group_name, f'Aggregated_existing_{group_name}_{grouping_}.csv'))
                self.tech_agg_existing[0].to_csv(os.path.join(self.nestor_ee_db_path, self.case_name, group_name,f'Aggregated_ts_existing_{group_name}_{grouping_}.csv'))

                
                df = self.tech_agg_existing[1]
                ts_df= self.tech_agg_existing[0]

                for x in range(grouping_):
                    if F'Wind_items_00{x}' in ts_df.columns:
                        self.grouped_timeseries[group_name +'_stock_'+ F'Wind_items_00{x}'] = ts_df[F'Wind_items_00{x}']  / df.loc[df['group_2014'] == F'Wind_items_00{x}']["capacity"].sum()
                
                # print(self.grouped_timeseries)
            except Exception as e:
                print(e)
            
            #National/ All federal state  Wind Technology aggregation 
            # national_aggregation_ = Technology.sim_wind(national_placements,
            #                     grouping_method="spagat",
            #                     n_groups=grouping_,
            #                     turbine=None,
            #                     year=2014,
            #                     offshore=False,                                
            #                 )
            # national_aggregation_[0]/national_capacity
        elif self.technology.endswith("pv"):
            #single region solar potentials Technology aggregation
            self.tech_agg_potential = Technology.sim_pv(potential_placements,
                                grouping_method="spagat",
                                n_groups=grouping_,
            ...
            for x in range(grouping_):
                if F'PV_items_00{x}' in ts_df.columns:
                    self.grouped_timeseries[group_name +'_'+ F'PV_items_00{x}'] = ts_df[F'PV_items_00{x}']  / df.loc[df['group_2014'] == F'PV_items_00{x}']["capacity"].sum()
            
            #single region solar (existing) Technology aggregation and Timer series output
            self.tech_agg_existing = Technology.sim_pv(
                            placements,
                            grouping_method="spagat",
                            n_groups=grouping_,
                            module="LG Electronics LG370Q1C-A5",
            ...
            self.tech_agg_existing[1].to_csv(os.path.join(self.nestor_ee_db_path, self.case_name, group_name, f'Aggregated_existing_{group_name}_{grouping_}.csv'))
            self.tech_agg_existing[0].to_csv(os.path.join(self.nestor_ee_db_path, self.case_name, group_name, f'Aggregated_ts_existing_{group_name}_{grouping_}.csv'))
            print(f'done in {self.case_name}..{group_name}')
            ...
            for x in range(grouping_):
                if F'PV_items_00{x}' in ts_df.columns:
                    self.grouped_timeseries[group_name +'_stock_'+ F'PV_items_00{x}'] = ts_df[F'PV_items_00{x}']  / df.loc[df['group_2014'] == F'PV_items_00{x}']["capacity"].sum()
            # print(self.grouped_timeseries)

            #National/All federal state solar Technology aggregation
            # national_aggregation_= Technology.sim_pv(
            #                 national_placements,
            #                 grouping_method="spagat",
            #                 n_groups=grouping_,
            #                 module="LG Electronics LG370Q1C-A5",
            ...
            #             )
            # national_aggregation_[0]/national_capacity
    def spatial_aggregation_prepare_historical_data(self, group_name, lifetime):
        if self.technology=='offshore' or self.technology=='onshore':
            existing = self.tech_agg_existing[1]
            for x in range(grouping_):
                existing_ =  existing.loc[existing['group_2014'] == F'Wind_items_00{x}']
                if 'commissioning_year' in existing_.columns:
                    try:
                        commissioning_per_year = existing_.groupby(
                            "commissioning_year").sum()["capacity"]
                        ...
                        grouped_historical_data_regions = historical_stock/1000000
                    
                        self.grouped_historical_data_regions[str(group_name)+'_'+ F'Wind_items_00{x}'] = grouped_historical_data_regions
                    except:
                        pass
                else:
                    self.grouped_historical_data_regions[str(group_name)+'_'+ F'Wind_items_00{x}'] = 0
            # print(self.grouped_historical_data_regions)
        elif self.technology=='openfield_pv' :
            existing = self.tech_agg_existing[1]
            for x in range(grouping_):
                existing_ =  existing.loc[existing['group_2014'] == F'PV_items_00{x}']
                if 'commissioning_year' in existing_.columns:
                    try:
                        commissioning_per_year = existing_.groupby(
                            "commissioning_year").sum()["capacity"]
                        ...
                        self.grouped_historical_data_regions[str(group_name)+'_'+ F'PV_items_00{x}'] = grouped_historical_data_regions
                    except:
                        pass
                else:
                    self.grouped_historical_data_regions[str(group_name)+'_'+ F'Wind_items_00{x}'] = 0
            ...
            pass
            # for x in range(grouping_):
            #     self.grouped_historical_data_regions[str(group_name)+'_'+ F'PV_items_00{x}'] = self.grouped_historical_data_regions/1000000

    def spatial_aggregation_prepare_potentials(self, region, group_name):
        if self.technology=='offshore' or self.technology=='onshore':
            plant_capacity =  self.tech_agg_potential[1]
            for x in range(grouping_):
                capacity = plant_capacity.loc[plant_capacity['group_2014'] == F'Wind_items_00{x}']["capacity"].sum()/1000000
                print(f'wind capacity only {group_name}_Wind_items_00{x}', capacity)
                
                capacity =  capacity +  self.grouped_historical_data_regions.loc[:, str(group_name)+'_'+ F'Wind_items_00{x}'].max()

                print(f'historical wind capacity 2020 {group_name}_Wind_items_00{x}', self.grouped_historical_data_regions.loc[2020, str(group_name)+'_'+ F'Wind_items_00{x}'])
                
                print('max historical + capacity wind ', capacity)
                # potential
                if self.region_availability[region]["potential"]:
                    
                    new_row = pd.DataFrame(
                        {"name": [group_name+'_'+ F'Wind_items_00{x}' ], "profile": [group_name+'_'+ F'Wind_items_00{x}' ],
                        "lb": [0], "ub": [capacity]})
                    self.grouped_potentials = pd.concat([self.grouped_potentials, new_row], axis=0)

                # existing
                if self.region_availability[region]["existing"]:
                    
                    new_row_existing = pd.DataFrame(
                        {"name": [group_name+"_stock_"+ F'Wind_items_00{x}'], "profile": [group_name+"_stock_"+ F'Wind_items_00{x}'],
                        "lb": [0], "ub": [0]})
                    self.grouped_potentials = pd.concat(
                        [self.grouped_potentials, new_row_existing], axis=0)
        elif self.technology=='openfield_pv' :
            try:
                plant_capacity =  self.tech_agg_potential[1]
                for x in range(grouping_):
                    if 'group_2014' in plant_capacity.columns:
                        capacity = plant_capacity.loc[plant_capacity['group_2014'] == F'PV_items_00{x}']["capacity"].sum()/1000000
                        print(f'Ofpv capacity {group_name}_PV_items_00{x}',capacity)

                        capacity = capacity +  self.grouped_historical_data_regions.loc[:, str(group_name)+'_'+ F'PV_items_00{x}'].max()

                        print(f'historical Ofpv capacity 2020 {group_name}_PV_items_00{x}',self.grouped_historical_data_regions.loc[2020, str(group_name)+'_'+ F'PV_items_00{x}'])

                        print('max historical + capacity ofpv ', capacity)
                        # capacity =  capacity +  self.grouped_historical_data_regions.loc[2020, str(group_name)+'_'+ F'PV_items_00{x}']
                        # print(f'after historical added to Ofpv capacity {group_name}_PV_items_00{x}',capacity, ' max capacity' , max_capacity)
                        # potential
                        if self.region_availability[region]["potential"]:
                            
                            new_row = pd.DataFrame(
                                {"name": [group_name+'_'+ F'PV_items_00{x}' ], "profile": [group_name+'_'+ F'PV_items_00{x}'],
                                "lb": [0], "ub": [capacity]})
                            self.grouped_potentials = pd.concat([self.grouped_potentials, new_row], axis=0)
                        ...
                        if self.region_availability[region]["existing"]:
                            ...
                            new_row_existing = pd.DataFrame(
                                {"name": [group_name+"_stock_"+ F'PV_items_00{x}'], "profile": [group_name+"_stock_"+ F'PV_items_00{x}'],
                                "lb": [0], "ub": [0]})
                            self.grouped_potentials = pd.concat(
                                [self.grouped_potentials, new_row_existing], axis=0)
            except:
    ...
    def _exclude_existing(self, rs, *argv,**kwargs): #technology
        ...
        if self.technology == 'onshore':
        ...
        elif self.technology == 'openfield_pv' and self.case_name.startswith('paper_SQR'): 
            ...
            if all([os.path.isfile(os.path.join(nestor_ee_result_path, f)) for f in [f"OpenfieldPV_{rs}.csv", "report.json", "OpenfieldPV_potential_area.tif", "OpenfieldPV_potential_items.shp"]]):
                ...
                if 'commissioning_year' not in trp.OpenfieldPV.existing_items.columns:
                    trp.OpenfieldPV.existing_items['commissioning_year']= pd.to_datetime(trp.OpenfieldPV.existing_items['ENH_InbetriebnahmeDatum']).dt.year
        ...
        elif self.technology == 'openfield_pv' and self.case_name.startswith('RoadsRailways'): 
            ...
            # check if existing, where already excluded
            report_path = os.path.join(trp.OpenfieldPVRoads.result_path, "report.json")
            with open(report_path) as f:
                report = json.load(f)
            ...
            if new_case_name is None:
                nestor_ee_result_path = os.path.join(
                    self.nestor_ee_db_path, self.case_name, f'OpenfieldPVRoads_{rs}')
            else:
                nestor_ee_result_path = os.path.join(
                    self.nestor_ee_db_path, new_case_name, f'OpenfieldPVRoads_{rs}')
            print("nestor_ee_result_path")
            print(nestor_ee_result_path)
            if not os.path.isdir(nestor_ee_result_path):
                os.makedirs(nestor_ee_result_path)

            if all([os.path.isfile(os.path.join(nestor_ee_result_path, f)) for f in [f"OpenfieldPVRoads_{rs}.csv", "report.json", "OpenfieldPVRoads_potential_area.tif", "OpenfieldPVRoads_potential_items.shp"]]):
                print("All files here")
                pass
            else:
                trp.OpenfieldPVRoads._load_eligible_area()
                trp.OpenfieldPVRoads.estimate_potential(
                    update=False,
                    exclusion_dict={"existing": {
                        "ExclusionCriteria": trp.OpenfieldPVRoads.ec}},
                    ignore_db=False, predict=True, rerun=True )
                # columns = [capacity, lat, lon, azimuth, tilt, share, ID, generation, poa]

           

                trp.OpenfieldPVRoads.sim(**{"n_groups": 1})
                trp.OpenfieldPVRoads.sim_existing()
                
                
                
                trp.OpenfieldPVRoads.existing_items.to_csv(nestor_ee_result_path+f'/existing_OpenfieldPVRoads_{rs}.csv')
                trp.OpenfieldPVRoads.predicted_items.to_csv(nestor_ee_result_path+f'/OpenfieldPVRoads_{rs}.csv')
                trp.OpenfieldPVRoads.ts_existing_items.to_csv(nestor_ee_result_path+f'/ts_existing_OpenfieldPVRoads_{rs}_2014.csv')
                trp.OpenfieldPVRoads.ts_predicted_items.to_csv(nestor_ee_result_path+f'/ts_OpenfieldPVRoads_{rs}_2014.csv')
                if new_case_name is not None:
                    trp.case = new_case_name
                trp.db_path = self.nestor_ee_db_path
                trp.OpenfieldPVRoads.result_path = nestor_ee_result_path
                trp.to_db("OpenfieldPVRoads")
                trp.existing_to_db("OpenfieldPVRoads")

            print(trp.OpenfieldPVRoads.result_path)
            print(
                "Estimated potential after "
                + f"{(time.time() - start) / 60} minutes",
                flush=True,
